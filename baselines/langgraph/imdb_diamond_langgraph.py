...
def entry_movie(state: State, model_b: ChatOpenAI) -> Dict[str, Any]:
    ...
    entry_movie_db = call_db_worker_sync(
        "entry_movie",
        ENTRY_MOVIE_QUERIES,
        {"search_keyword": state["search_keyword"]},
    )
    prepared_entry_movie_db = json.dumps(_prepare_result_for_prompt(json.loads(entry_movie_db)), ensure_ascii=False)
    prompt = build_halo_prompt(
        None,
        {"user_query": state["user_query"], "search_keyword": state["search_keyword"]},
        [prepared_entry_movie_db],
    )
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="IMDb diamond LangGraph workflow (Halo-aligned prompts).")
    ...
    if args.query:
        ...
        if not queries:
            ...
            return 1
    metrics.start(run_name="imdb_diamond_langgraph", total_queries=len(queries))
