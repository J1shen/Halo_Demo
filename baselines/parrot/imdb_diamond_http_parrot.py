"""Baseline Parrot workflow for the IMDb diamond HTTP template (template-aligned).

- Mirrors the nodes from templates/imdb_diamond_http.yaml (imdb_qa_agent_diamond_http_v1).
...
async def run_one_query_async(user_query: str, idx: int | None = None, total: int | None = None) -> None:
    ...
    # ===== 2–4) entry fan-out =====
    # entry_movie_qwen14b
    entry_movie_payloads: Dict[str, Any] = {}
    for spec in ENTRY_MOVIE_HTTP_NODES:
        ...
        entry_movie_payloads[spec["output"]] = payload
    entry_movie_prompt = build_halo_prompt(
        None,
        {
            "user_query": uq,
            "search_keyword": search_keyword,
            **entry_movie_payloads,
        },
        [],
    )
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="IMDb diamond HTTP Parrot workflow (template-aligned; input/output only)."
    )
    ...
    metrics.start(run_name="imdb_diamond_http_parrot", total_queries=len(queries))
