"""Baseline Parrot workflow for the TPC-H fanout template (template-aligned).

- Mirrors the nodes from templates/tpch_fanout.yaml.
...
async def run_one_query_async(user_query: str, idx: int | None = None, total: int | None = None) -> None:
    ...
    s1_q3_db = await _call_db("stage1_seg_ship_q3", S1_Q3_QUERIES, tpch_params)
    s1_q3_prompt = build_halo_prompt(
        None,
        {"user_query": uq, "tpch_params": tpch_params},
        [_guard_input(s1_q3_db or "{}")],
    )
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="TPC-H fanout Parrot workflow (template-aligned).")
    ...
    metrics = BaselineMetrics()
    metrics.start(run_name="tpch_fanout_parrot", total_queries=len(queries))
