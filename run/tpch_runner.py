...
DEFAULT_INPUT_FILE = Path("data/tpch_user_inputs.txt")
HALO_PG_DBNAME = "tpch"
...
def main(argv: List[str] | None = None) -> None:
    ...
    metrics.start_run("tpch", len(queries))
    ...
    for user_query, initial_context, final_context in zip(queries, initial_contexts, final_contexts):
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    cli = argparse.ArgumentParser(description="Run TPC-H workflow over queries from a text file.")
    cli.add_argument(
        "-t",
        "--template",
        type=Path,
        default=Path("templates/tpch_trident.yaml"),
    ...
    cli.add_argument(
        ...
        help="Path to a text file containing one user_query per line.",
    )
    cli.add_argument(
        "-n",
        "--sample-count",
        type=int,
        default=1024,
        help="Number of user queries from the file to execute per run.",
    )
    cli.add_argument(
        "--plan-sample-count",
        type=int,
        default=4,
        help="Number of sampled queries from the file to use during plan optimization.",
    )
    cli.add_argument(
        "--max-batch-size",
        type=int,
...
def _load_queries_from_file(path: Path) -> List[str]:
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            queries.append(q)
    if not queries:
        raise RuntimeError(f"No non-empty queries found in {path}")
    return queries
...
def _prepare_queries(args: argparse.Namespace) -> tuple[List[Dict[str, Any]], List[str]]:
    ...
    queries = _load_queries_from_file(args.input_file)
    if len(queries) < total_needed:
        raise RuntimeError(
            f"Needed {total_needed} queries (plan + run), "
            f"but only {len(queries)} available in {args.input_file}."
        )

    plan_contexts = [{"user_query": q} for q in queries[:plan_needed]]
    run_queries = queries[: run_needed or len(queries)]

    _log(
        f"Loaded {len(queries)} querie(s) from {args.input_file}; "
        f"using {len(plan_contexts)} for planning and {len(run_queries)} for execution."
    )
    return plan_contexts, run_queries


def _log(message: str) -> None:
    print(f"[TPCHRunner] {message}")
